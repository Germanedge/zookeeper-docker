...
if [[ ${current_uid} != 1000 ]] || [[ ${current_gid} != 1000 ]]; then
        cat <<EOF
        ********************************************************************************
            BREAKING CHANGE: Data files are inaccessable! For the new version of
                             the zookeeper container, all data files (/opt/zookeeper/data) 
                             must be owned by 1000:1000.
                             
                             Currently they are owned by ${current_uid}:${current_gid}.
                             
                             The container will continue to run.
                             Please correct the ownership of the files and
